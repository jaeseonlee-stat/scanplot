...
#' and plot the result for each time point to detect hot-spots.
#'
  ...
  for (i in 1:(ncol(data))) {
    ...
    if (length(cluster2)!=0) {
      id[[temp]] <- data.frame(week=rep(colnames(data)[i],length(cluster2)),
                               sido=shp.name[cluster2, 1], sigungu=shp.name[cluster2, 2],
    ...
    jpeg(paste("cluster_",colnames(data)[i],".jpeg",sep = ""), res=600, width=200, height=150, pointsize=9,units='mm')
    ...
    title(paste("Cluster map for", colnames(data)[i], sep=" "))
